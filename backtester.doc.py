...
#           CONFIGURATION
# ────────────────────────────────────────────────
# Central place for all strategy parameters and file paths.
# Easy to tweak lookback periods, tickers, capital, date range, etc.
...
# ────────────────────────────────────────────────
# Pulls dividend- and split-adjusted closing prices from Yahoo Finance.
# Uses auto_adjust=True so we don't have to manually handle dividends/splits.
# Downloads a longer period than the analysis window so momentum calculations
# have enough history right from the first month.
...
# ────────────────────────────────────────────────
# Identifies the actual last trading day ≤ the calendar month-end date
# for each month in the analysis window. This becomes our rebalance dates.
# We use pd.date_range with freq="ME" to get calendar month-ends,
# then backfill to the nearest prior trading day.
...
# ────────────────────────────────────────────────
# For every month-end date and every ticker:
#   - Calculate momentum (gain %) over 63, 126, 252 trading days
#   - Compute "Papa Avg" = simple average of the three momentum values
#   - Store all intermediate values (past dates, closes, gains)
# Only includes rows where we have clean data for **all** lookback periods.
...
for as_of_date in monthly_ends:
    ...
    for ticker in TICKERS:
        ...
        if not valid:
            continue  # skip this ticker/date — not enough clean history
...
# Sort: date asc, then Papa Avg desc within date
...
# ────────────────────────────────────────────────
# From the ranked history data, take the top 3 momentum tickers each month.
# This becomes the "target" holdings for each rebalance date.
# Saved separately so it's easy to audit what the strategy wanted to hold.
...
# ────────────────────────────────────────────────
# Core backtest logic: we maintain **three independent buckets** (ranks 1,2,3)
# Each bucket starts with ~$33,333 and only reinvests within its own rank.
# Rules:
#   - If the same ticker stays in the same rank → keep the same share count
#   - If the ticker changes → sell old holding at current close, buy as many
#     shares as possible of the new ticker with the proceeds (integer shares)
# This avoids unnecessary trading when a good performer stays in its rank.
