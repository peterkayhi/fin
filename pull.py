...
if __name__ == "__main__":
    ...
    result = calculate_nominal_total_gain(symbol, start, end)
    
    if "error" in result:
        print("Error:", result["error"])
    else:
        print(f"\nNominal Total Gain for {result['symbol']}")
        print(f"Period: {result['actual_start_date']} → {result['actual_end_date']}")
        print(f"Adjusted Start Price: ${result['start_price_adj']:.2f}")
        print(f"Adjusted End Price:   ${result['end_price_adj']:.2f}")
        print(f"Total Gain:            {result['total_gain_percentage']:+.2f}%")
        print(f" (multiplier: ×{result['total_gain_multiplier']:.3f})")
